<template>
  <div class="report-page">
    <common-table
      ref="tableRef"
      :columns="columns"
      :api-method="reportPageApi.getPageReportList"
...
import * as reportPageApi from '@/api/report/report-page'
...
const handleDelete = (row) => {
  ElMessageBox.confirm('确定要删除该报告吗？', '提示', {
    ...
    try {
      const res = await reportPageApi.deleteReportPage(row.id)
...
const handleBatchDelete = async (rows) => {
  ElMessageBox.confirm(`确定要删除选中的 ${rows.length} 个报告吗？`, '提示', {
    ...
    try {
      ...
      const res = await reportPageApi.batchDeleteReportPage(ids)
