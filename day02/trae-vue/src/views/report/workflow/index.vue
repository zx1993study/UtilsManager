<template>
  <div class="report-workflow">
    <common-table
      ref="tableRef"
      :columns="columns"
      :api-method="reportWorkflowApi.getWorkflowReportList"
...
import * as reportWorkflowApi from '@/api/report/report-workflow'
...
const handleDelete = (row) => {
  ElMessageBox.confirm('确定要删除该报告吗？', '提示', {
    ...
    try {
      const res = await reportWorkflowApi.deleteReportWorkflow(row.id)
...
const handleBatchDelete = async (rows) => {
  ElMessageBox.confirm(`确定要删除选中的 ${rows.length} 个报告吗？`, '提示', {
    ...
    try {
      ...
      const res = await reportWorkflowApi.batchDeleteReportWorkflow(ids)
