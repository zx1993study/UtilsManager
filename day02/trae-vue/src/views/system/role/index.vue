<template>
  <div class="role-management">
    <common-table
      ref="tableRef"
      :columns="columns"
      :api-method="roleApi.getRoleList"
...
const handleBatchDelete = async (rows) => {
  ElMessageBox.confirm(`确定要删除选中的 ${rows.length} 个角色吗？`, '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(async () => {
    try {
      ...
      const res = await roleApi.batchDeleteRole(ids)
...
const handleSubmit = async () => {
  ...
  try {
    ...
    let failMsg
    if (formData.id) {
      res = await roleApi.updateRole(formData)
      okMsg = '编辑成功'
      failMsg = '编辑失败'
    } else {
      res = await roleApi.addRole(formData)
      okMsg = '新增成功'
      failMsg = '新增失败'
    }
