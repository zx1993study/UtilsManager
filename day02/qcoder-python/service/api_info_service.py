...
from schemas.api_info_schemas import ApiInfoCreate, ApiInfoUpdate, ApiInfoInfo, ApiInfoList, ApiInfoOptionQuery
from mysql.api_info_sql import (
    get_api_info_by_id,
    get_api_info_by_unique_fields,
    get_api_info_list,
    get_api_info_options,
    create_api_info,
    update_api_info,
    delete_api_info
)
...
def _build_api_payload(data, exclude_unset: bool = False):
...
async def get_api_info_options_service(db: Session, data: ApiInfoOptionQuery):
    items = await get_api_info_options(db, data)
...
async def create_api_info_service(db: Session, data: ApiInfoCreate):
    ...
    payload, token_ids = _build_api_payload(data)
    obj = await create_api_info(db, payload)
    await set_api_token_infos(db, obj.api_id, token_ids)
    
    """将ORM对象转换为Schema对象"""
    schema_obj = await get_api_info_by_id(db, obj.api_id)
...
async def update_api_info_service(db: Session,  data: ApiInfoUpdate):
    """更新接口信息"""
    """校验是否存在"""
    payload, token_ids = _build_api_payload(data, exclude_unset=True)
    obj = await update_api_info(db, data.api_id, payload)
    if not obj:
        return error_response(
            msg="更新失败，信息不存在",
            data=None,
