"""
页面实例CRUD操作
Generated by scaffold script
"""
from sqlalchemy.orm import Session
from sqlalchemy import and_, func
from models.page_instance_model import PageInstance
from models.token_info_model import TokenInfo
from datetime import datetime
from typing import List, Optional, Tuple
from utils.data_paser import set_audit_fields_for_create, set_audit_fields_for_update


async def get_page_instance_by_id(db: Session, page_instance_id: int) -> Optional[PageInstance]:
    """根据ID获取页面实例"""
    return db.query(PageInstance).filter(PageInstance.page_instance_id == page_instance_id).first()


async def get_page_instance_by_unique_fields(
    db: Session, 
    instance_name: str, 
    page_id: int
) -> Optional[PageInstance]:
    """根据唯一字段组合获取页面实例
    
    Args:
        db: 数据库会话
        instance_name: 实例名称
        page_id: 页面ID
        
    Returns:
        Optional[PageInstance]: 如果存在则返回页面实例，否则返回None
    """
    return db.query(PageInstance).filter(
        and_(
            PageInstance.instance_name == instance_name,
            PageInstance.page_id == page_id
        )
    ).first()


async def get_page_instance_list(
    db: Session,
    page_num: int = 1,
    page_size: int = 10,
    page_id: Optional[int] = None
) -> Tuple[List[dict], int]:
    """获取页面实例分页列表

    Args:
        page_id: 可选，按页面ID过滤（详情页只看本页面的实例）

    Returns:
        Tuple[List[PageInstance], int]: (数据列表, 总记录数)
    """
    """ 计算偏移量""" 
    offset = (page_num - 1) * page_size

    """基础查询，按需过滤 page_id"""
    base_query = db.query(PageInstance)
    if page_id is not None:
        base_query = base_query.filter(PageInstance.page_id == page_id)

    """查询总数"""
    total = base_query.with_entities(func.count(PageInstance.page_instance_id)).scalar()

    """查询分页数据"""
    rows = db.query(
        PageInstance,
        TokenInfo.name.label("token_name")
    ).outerjoin(
        TokenInfo, TokenInfo.token_id == PageInstance.token_id
    )
    if page_id is not None:
        rows = rows.filter(PageInstance.page_id == page_id)
    rows = rows.order_by(PageInstance.page_instance_id.desc()).offset(offset).limit(page_size).all()

    items = []
    for instance, token_name in rows:
        items.append({
            "page_instance_id": instance.page_instance_id,
            "page_id": instance.page_id,
            "token_id": instance.token_id,
            "operation_json": instance.operation_json,
            "instance_name": instance.instance_name,
            "screen_photo_file": instance.screen_photo_file,
            "description": instance.description,
            "expect_result": instance.expect_result,
            "exec_count": instance.exec_count,
            "status": instance.status,
            "remark": instance.remark,
            "creator": instance.creator,
            "create_time": instance.create_time,
            "update_time": instance.update_time,
            "token_name": token_name,
        })

    return items, total


async def get_page_instances_by_ids(db: Session, ids: List[int]) -> List[PageInstance]:
    """根据页面实例ID列表批量获取页面实例"""
    if not ids:
        return []
    return db.query(PageInstance).filter(
        PageInstance.page_instance_id.in_(ids)
    ).order_by(PageInstance.page_instance_id.desc()).all()


async def get_page_instances_by_page_ids(db: Session, page_ids: List[int]) -> List[PageInstance]:
    """根据页面ID列表获取其下的全部页面实例"""
    if not page_ids:
        return []
    return db.query(PageInstance).filter(
        PageInstance.page_id.in_(page_ids)
    ).order_by(PageInstance.page_id.desc(), PageInstance.page_instance_id.desc()).all()


async def create_page_instance(db: Session, data: dict) -> PageInstance:
    """创建页面实例"""
    data.setdefault("exec_count", 0)
    data.setdefault("status", 0)
    data = set_audit_fields_for_create(data)
    db_obj = PageInstance(**data)
    db.add(db_obj)
    db.commit()
    db.refresh(db_obj)
    return db_obj


async def update_page_instance(db: Session, page_instance_id: int, data: dict) -> Optional[PageInstance]:
    """更新页面实例"""
    db_obj = db.query(PageInstance).filter(PageInstance.page_instance_id == page_instance_id).first()
    if db_obj:
        data = set_audit_fields_for_update(data)
        for key, value in data.items():
            setattr(db_obj, key, value)
        db.commit()
        db.refresh(db_obj)
    return db_obj


async def update_page_instance_execute_state(
    db: Session,
    page_instance_id: int,
    success: bool,
    screen_photo_file: Optional[str] = None,
) -> Optional[PageInstance]:
    """更新页面实例最近一次执行状态、截图和执行次数。"""
    db_obj = db.query(PageInstance).filter(PageInstance.page_instance_id == page_instance_id).first()
    if db_obj:
        db_obj.exec_count = (db_obj.exec_count or 0) + 1
        db_obj.status = 1 if success else 2
        if screen_photo_file is not None:
            db_obj.screen_photo_file = screen_photo_file
        db_obj.update_time = datetime.now()
        db.commit()
        db.refresh(db_obj)
    return db_obj


async def delete_page_instance(db: Session, page_instance_id: int) -> Optional[PageInstance]:
    """删除页面实例"""
    db_obj = db.query(PageInstance).filter(PageInstance.page_instance_id == page_instance_id).first()
    if db_obj:
        db.delete(db_obj)
        db.commit()
    return db_obj


async def delete_page_instance_batch(db: Session, ids: List[int]) -> int:
    """根据页面实例ID列表批量删除，返回删除条数"""
    if not ids:
        return 0
    count = db.query(PageInstance).filter(
        PageInstance.page_instance_id.in_(ids)
    ).delete(synchronize_session=False)
    if count:
        db.commit()
    return count


async def delete_page_instance_by_page_ids(db: Session, page_ids: List[int]) -> int:
    """根据页面ID列表删除页面实例（手动级联），返回删除条数"""
    if not page_ids:
        return 0
    count = db.query(PageInstance).filter(
        PageInstance.page_id.in_(page_ids)
    ).delete(synchronize_session=False)
    if count:
        db.commit()
    return count
